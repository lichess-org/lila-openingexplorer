...
pub struct DbOpt {
    ...
    db_cache: usize,
    /// Estimate for average size of cache entry. Best tuned by looking
    /// at real production data, and dividing cache size by occupancy.
    #[arg(long, default_value = "7610")]
    db_estimated_cache_entry_charge: usize,
...
impl Column<'_> {
    fn descriptor(self) -> ColumnFamilyDescriptor {
        ...
        table_opts.set_block_size(64 * 1024); // Relatively large, for spinning disks
...
impl Database {
    pub fn open(opt: DbOpt) -> Result<Database, rocksdb::Error> {
        ...
        let cache = Cache::new_hyper_clock_cache(opt.db_cache, opt.db_estimated_cache_entry_charge);
